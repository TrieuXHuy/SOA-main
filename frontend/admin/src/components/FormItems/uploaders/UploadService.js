...
export default class FileUploader {
  ...
  static async uploadToServer(file, path, filename) {
    ...
    // Use the new MinIO upload endpoint: /upload/{productCode}
    const uri = `${config.baseURLApi}/upload/${path}`;
    ...
    // Return the URL from MinIO
    return response.data.url || response.data;
  }

  static async deleteImage(imageUrl) {
    try {
      const uri = `${config.baseURLApi}/upload/by-url?url=${encodeURIComponent(imageUrl)}`;
